...
export class JsPDFComponent implements OnInit, OnDestroy {
  ...
  constructor(
   ...
   pf(download?: any, post?: any): void {
    ...
    sparador.src = `http://192.168.2.27:3000/files/separador.png`;
    sparador.alt = 'alt';

    fb.src = `http://192.168.2.27:3000/files/facebook.png`;
    fb.alt = 'alt';
    ...
    if (post) {
      this.adminService.postContract(this.id, date, doc.output('blob')).subscribe(response => {
        ...
        this.adminService.getContract(this.id).subscribe( response => {
          ...
          this.pdfSrc = `http://192.168.2.27:3000/files/${response['nombre']}`;
        });
      });
    }
  ...
  rif(download?: any, post?: any) {
    ...
    if (post) {
      this.adminService.postContract(this.id, date, doc.output('blob')).subscribe(response => {
        this.contratoName = response['nombre'];
        localStorage.setItem('cont', response['nombre']);
        this.adminService.getContract(this.id).subscribe( response => this.pdfSrc = `http://192.168.2.27:3000/files/${response['nombre']}`);
      });
    }
  }
...
// ======================================================================================================================= //
    ...
    if (post) {
      ...
      this.adminService.postReceipt(this.id, identificador, date, id_payment, doc.output('blob')).subscribe(response => {
        this.adminService.getReceiptById(response['id_receipt']).subscribe( response => {
          ...
          this.recibov = `http://192.168.2.27:3000/files/${response[0]['name']}`;
        });
      });
    }
  }
  ...
  prereceipt(download?: any, post?: any) {
    ...
    magua.src = `http://192.168.2.27:3000/files/images/magua.png`;
    magua.alt = 'alt';
...
// ======================================================================================================================= //
  ...
  async ngOnInit() {
    ...
    if (localStorage.getItem('post')) {
      this.recibov = `http://192.168.2.27:3000/files/${localStorage.getItem('rec')}`;
      if (this.inf.id_service == 1) {
        this.pdfSrc = `http://192.168.2.27:3000/files/${localStorage.getItem('cont')}`;
      }
      if (this.inf.id_service == 2) {
        this.pdfSrc = `http://192.168.2.27:3000/files/${localStorage.getItem('cont')}`;
      }
