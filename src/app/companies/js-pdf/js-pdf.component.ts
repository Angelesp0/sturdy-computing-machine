...
export class JsPDFComponent implements OnInit, OnDestroy {
  ...
  constructor(
   ...
   pf(download?: any, post?: any): void {
    ...
    sparador.src = `http://74.208.71.98:3000/files/separador.png`;
    sparador.alt = 'alt';

    fb.src = `http://74.208.71.98:3000/files/facebook.png`;
    fb.alt = 'alt';
    ...
    if (post) {
      this.adminService.postContract(this.id, date, doc.output('blob')).subscribe(response => {
        ...
        this.adminService.getContract(this.id).subscribe( response => {
          ...
          this.pdfSrc = `http://74.208.71.98:3000/files/${response['nombre']}`;
        });
      });
    }
  ...
  rif(download?: any, post?: any) {
    ...
    if (post) {
      this.adminService.postContract(this.id, date, doc.output('blob')).subscribe(response => {
        this.contratoName = response['nombre'];
        localStorage.setItem('cont', response['nombre']);
        this.adminService.getContract(this.id).subscribe( response => this.pdfSrc = `http://74.208.71.98:3000/files/${response['nombre']}`);
      });
    }
  }
    ...
    if (post) {
      ...
      this.adminService.postReceipt(this.id, identificador, date, id_payment, doc.output('blob')).subscribe(response => {
        this.adminService.getReceiptById(response['id_receipt']).subscribe( response => {
          ...
          this.recibov = `http://74.208.71.98:3000/files/${response[0]['name']}`;
        });
      });
    }
  }

  prereceipt(download?: any, post?: any) {
    ...
    const testigo2 = new Image();
    const logo = new Image();
    const magua = new Image();
    const columns = [['Año', 'Mes', 'Total',  'Estado']];
    const data = [[this.todayDate.getFullYear(), this.todayDate.getMonth() + 1, this.value, 'En verificacion']];
    const value2: number = +this.value;
    const value3 = nl(value2);
    const a = this.value;
    const b = parseFloat(this.value);
    ...
    magua.src = `http://74.208.71.98:3000/files/images/magua.png`;
    magua.alt = 'alt';
    ...
    doc.setFont('helvetica', 'bold');
    // doc.setFontType();
    doc.text('P R E - R E C I B O   D E   P A G O', 190, 120);
    doc.setFont('Arial', 'bold');
...
// ======================================================================================================================= //
    ...
    doc.text(` ${(this.inf.razon).toUpperCase()} `, 150, 223, );
    // ======================================================================================================================= //
    // doc.setFontType("bold");
    doc.setFont('Arial', 'bold');
    ...
    doc.setFontSize(9);
    doc.text(`Gestoria Empresarial Global Service SC, implementa las medidas de seguridad y mecanismos tecnicos necesarios para la proteccion de datos personales y sencibles, procurando siempre la integridad de dichos datos evitando asi el daño, perdida, o el uso, acceso a tratamiento no autorizado por los titulares, la vulnerabilidad surgida por la alteracion de los mismos sera informada por Gestoria Empresarial Global Service SC, en la brevedad posible a los titulares, a fin de que se puedan tomar medidas pertinentes a la defensa de sus derechos tal como lo señala el articulo 20 de la ley, Gestoria Empresarial Global Service SC, por consucto de sus representantes legales, directivos o personal a cargo se compromete a guardar confidencialidad respuecto a los datos de los titulares, confidencialidad que subsistira aun despues de terminada la relacion con Gestoria Empresarial Global Service SC, teniendo asi prohinido el acceso de personas no autorizadas y utilizar los datos personales de los titulares para los fines distintos establecidos de manera contractual o a los establecidos en el presente aviso de privacidad, para visualizar nuestro abiso de privacidad vigente, visitar la pagina de https://gglobals.com.mx/aviso-de-privacidad/` , 40, 640,  {maxWidth: 540, align: 'justify'});
    doc.addImage(magua, 'PNG', 50, 80, 516, 579);
  ...
  acuse(email: any, download?: any, post?: any): void {
    ...
    doc.text(`Manifestamos haciendo de su conocimiento que la contraseña y clave de acceso a su aplicacion (GGlobals), desarrollada por Gestoria Empresarial Global Service S. C, que le proporcionara nuestro ejecutivo de atención a empresas, por medio de este documento, es personal e intransferible, y que usted: ${(this.inf.first_name).toUpperCase()} ${(this.inf.last_name).toUpperCase()} con RFC: ${this.inf.rfc.toUpperCase()}, es responsable del resguardo y uso de las mismas`, 40, 180,  {maxWidth: 530, align: 'justify'});
    doc.text('La aplicacion Gglobals, es un sitio diseñado exclusivamente con el fin de establecer comunicacion de manera directa con cada uno de nuestros clientes del departamento de contabilidad. Por este medio podras realizar las siguientes consultas:' , 40, 240, {maxWidth: 530, align: 'justify'});
    // doc.setFontType("bold");
    doc.setFont('Arial', 'bold');
    ...
    doc.text( 'Para su uso correcto se encuentra disponible en nuestra página de internet: www.gglobals.com.mx/appglobals, un tutorial y manual de uso. Dicha aplicación, se encontrara vigente en las plataformas para sistemas operativos, ANDROID E IOS, y podra descargarse de manera gratuita.', 40, 330, {maxWidth: 530, align: 'justify'});
    doc.text('Para su ACCESO correspondiente, solo es necesario ingresar la clave de acceso y contraseña que a continuación se asigna a su empresa:' , 40, 370,  {maxWidth: 530, align: 'justify'});
    ...
    doc.text('Nota: si por algun motivo su clave de acceso o contraseña se extravia, no la recuerda, o las credenciales se dañen, para reestablecerla es necesario reportar de la siguiente manera: ' , 40, 450, {maxWidth: 530, align: 'justify'});
    doc.text('1) Enviar el reporte por medio de whatsapp al numero: (614) 163-88-17, o al correo electronico: servicioconta@gglobals.com, indicando en asunto "Reestablecer contraseña aplicación".' , 70, 480, {maxWidth: 530, align: 'justify'});
    doc.text('2) Debera ingresar los siguientes datos en su reporte: Nombre Completo de titular, número de contrato (número localizado en la parte superior derecha de su contrato, o en la parte media derecha de cualquier recibo de pago)' , 70, 510, {maxWidth: 530, align: 'justify'});
    doc.text('3) En un lapso no mayor a 1 hora se comunicara uno de nuestros ejecutivos empresariales a los medios de contacto establecidos en su registro indicandole el estatus y resolución de su reporte.' , 70, 540,  {maxWidth: 530, align: 'justify'});

    doc.setFontSize(11.5);
    doc.text(`Aviso de Privacidad Simplificado` , 240, 590, {maxWidth: 530, align: 'justify'});
    doc.setFontSize(9);
    doc.text(`Gestoria Empresarial Global Service SC, implementa las medidas de seguridad y mecanismos tecnicos necesarios para la protección de datos personales y sensibles, procurando siempre la integridad de dichos datos evitando asi el daño, perdida, o el uso, acceso a tratamiento no autorizado por los titulares, la vulnerabilidad surgida por la alteración de los mismos sera informada por Gestoria Empresarial Global Service SC, en la brevedad posible a los titulares, a fin de que se puedan tomar medidas pertinentes a la defensa de sus derechos tal como lo señala el articulo 20 de la ley, Gestoria Empresarial Global Service SC, por consucto de sus representantes legales, directivos o personal a cargo se compromete a guardar confidencialidad respuecto a los datos de los titulares, confidencialidad que subsistira aun después de terminada la relación con Gestoria Empresarial Global Service SC, teniendo asi prohibido el acceso de personas no autorizadas y utilizar los datos personales de los titulares para los fines distintos establecidos de manera contractual o a los establecidos en el presente aviso de privacidad, para visualizar nuestro aviso de privacidad vigente, visitar la pagina de internet: https://gglobals.com.mx/aviso-de-privacidad/` , 40, 610,  {maxWidth: 530, align: 'justify'});
  ...
  async ngOnInit() {
    ...
    if (localStorage.getItem('post')) {
      this.recibov = `http://74.208.71.98:3000/files/${localStorage.getItem('rec')}`;
      if (this.inf.id_service == 1) {
        this.pdfSrc = `http://74.208.71.98:3000/files/${localStorage.getItem('cont')}`;
      }
      if (this.inf.id_service == 2) {
        this.pdfSrc = `http://74.208.71.98:3000/files/${localStorage.getItem('cont')}`;
      }
      ...
      if (this.methodPayment === 'TRANSFERENCIA') {
        await this.prereceipt('false', 'post');
      } else {
  ...
  sendemail() {
    ...
    this.email.razon = this.inf.razon;
